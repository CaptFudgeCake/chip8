...
impl Command for ShiftRight {
    fn execute(&self, emulator: &mut Chip8) {
        if emulator.use_old_bit_shift {
            emulator.registers[self.register_x as usize] = emulator.registers[self.register_y as usize] >> 1;
            ...
            emulator.registers[self.register_x as usize] = emulator.registers[self.register_x as usize] >> 1;
        }
    }
}